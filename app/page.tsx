...
const stripImages = [
  '/assets/multimorph-regimes.png',
  '/assets/compressor-coupled-stage.png',
  '/assets/v4-engine.png',
  '/assets/aero-airfoil-aoa10.png',
]
    ...
    <main>
      <section className="gridline border-b rule">
        <div className="shell grid min-h-[82vh] items-center gap-12 py-16 lg:grid-cols-[.92fr_1.08fr] lg:py-20">
          <div>
            ...
            <p className="mt-6 max-w-2xl text-xl text-zinc-200">{t.home.role}</p>
            <p className="mt-6 max-w-2xl text-zinc-400">{t.home.intro}</p>
            ...
            <div className="mt-8 flex flex-wrap gap-3 text-sm">
              <Link href="#projects" className="bg-zinc-100 px-5 py-3 font-medium text-black transition hover:bg-white">{t.home.explore}</Link>
          ...
          <div className="hero-visual">
            <div className="hero-main relative overflow-hidden border rule">
              ...
              <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 bg-gradient-to-t from-black/90 via-black/40 to-transparent px-5 pb-4 pt-16">
                <div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-sky-200">{t.home.featured}</p><p className="mt-1 text-sm text-zinc-100">{t.home.featuredTitle}</p></div>
      ...
      <section className="border-b rule py-8">
        <div className="shell grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {stripImages.map((src, i) => {
            const [label, meta] = t.home.strip[i]
            return (
              <div key={src} className="group relative aspect-[16/9] overflow-hidden border rule bg-white/[0.02] shadow-[0_20px_55px_rgba(0,0,0,0.16)] backdrop-blur-sm">
                <Image src={src} alt={label} fill className="object-cover transition duration-500 group-hover:scale-[1.02]" sizes="(max-width:640px) 100vw, 25vw" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent" />
                <div className="absolute inset-x-0 bottom-0 p-3">
                  <p className="text-sm font-medium">{label}</p>
                  <p className="font-mono text-[10px] uppercase tracking-[.12em] text-zinc-400">{meta}</p>
                </div>
              </div>
            )
          })}
      ...
      <section id="projects" className="shell py-20">
        <div className="mb-10 grid gap-6 md:grid-cols-[1fr_1fr] md:items-end">
          <div><p className="eyebrow">{t.home.selectedEyebrow}</p><h2 className="mt-3 text-4xl font-semibold tracking-[-.045em] md:text-5xl">{t.home.selectedTitle}</h2></div>
          <p className="max-w-xl text-zinc-400 md:justify-self-end">{t.home.selectedBody}</p>
        </div>
      ...
      <section id="technical-work" className="border-y rule py-20">
        <div className="shell">
          <p className="eyebrow">{t.home.studiesEyebrow}</p>
          <div className="mt-4 flex items-end justify-between gap-8">
            <h2 className="text-4xl font-semibold tracking-[-.04em]">{t.home.studiesTitle}</h2>
            <p className="hidden max-w-md text-sm text-zinc-400 md:block">{t.home.studiesBody}</p>
          </div>
          <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">{studies.map(s => <StudyCard key={s.slug} study={s} />)}</div>
        </div>
      </section>

      <section className="shell py-24">
        <div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr]">
          <div><p className="eyebrow">{t.home.profileEyebrow}</p><h2 className="mt-3 text-4xl font-semibold tracking-[-.04em]">{t.home.profileTitle}</h2></div>
          <div className="grid gap-8 sm:grid-cols-2">
            {t.home.profileGroups.map(([title, body]) => <div key={title} className="border-t rule pt-4"><h3 className="font-semibold">{title}</h3><p className="mt-2 text-sm text-zinc-400">{body}</p></div>)}
          </div>
        </div>
      </section>
      ...
      <section className="border-t rule py-24">
        <div className="shell grid gap-8 md:grid-cols-[1fr_auto] md:items-end">
          <div><p className="eyebrow">{t.home.contactEyebrow}</p><h2 className="mt-3 max-w-3xl text-4xl font-semibold tracking-[-.04em]">{t.home.contactTitle}</h2></div>
          <div className="flex flex-wrap gap-3"><a href="/docs/alexandre-sigiridis-schlepp-cv.pdf" target="_blank" rel="noreferrer" className="border rule px-5 py-3 text-sm">CV PDF</a><a href="https://www.linkedin.com/in/alexandre-sigiridis-schlepp/" target="_blank" rel="noreferrer" className="border rule px-5 py-3 text-sm">LinkedIn</a><a href="mailto:sigischlepp.alex@gmail.com" className="bg-zinc-100 px-5 py-3 text-sm font-medium text-black">{t.home.email}</a></div>
        </div>
      </section>
    </main>
  )
